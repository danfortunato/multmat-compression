function coeffs = chebvals2chebcoeffs(values)
%CHEBVALS2CHEBCOEFFS   Convert values at Chebyshev points to Chebyshev coefficients.
%   C = VALS2COEFFS(V) returns the (N+1)x1 vector of coefficients such that
%   F(x) = C(1)*T_0(x) + C(2)*T_1(x) + C(N+1)*T_N(x) (where T_k(x) denotes
%   the k-th 1st-kind Chebyshev polynomial) interpolates the data
%   [V(1) ; ... ; V(N+1)] at Chebyshev points of the 2nd kind.
%
%   If the input V is an (N+1)xM matrix, then C = VALS2COEFFS(V) returns
%   the (N+1)xM matrix of coefficients C such that F_j(x) = C(1,j)*T_0(x) +
%   ... + C(N+1,j)*T_N(x) interpolates [V(1,j) ; ... ; V(N+1,j)] for
%   j = 1:M.
%
% See also CHEBCOEFFS2CHEBVALS.
