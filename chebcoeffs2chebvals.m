function values = chebcoeffs2chebvals(coeffs)
%CHEBCOEFFS2CHEBVALS   Convert Chebyshev coefficients to values at Chebyshev points.
%   V = CHEBCOEFFS2CHEBVALS(C) returns the values of the polynomial
%   V(i,1) = P(x_i) = C(1,1)*T_{0}(x_i) + ... + C(N,1)*T_{N-1}(x_i), where
%   the x_i are 2nd-kind Chebyshev nodes.
%
%   If the input C is an (N+1)xM matrix then V = CHEBCOEFFS2CHEBVALS(C)
%   returns the (N+1)xM matrix of values V such that V(i,j) = P_j(x_i) =
%   C(1,j)*T_{0}(x_i) + C(2,j)*T_{1}(x_i) + ... + C(N,j)*T_{N-1}(x_i).
%
% See also CHEBVALS2CHEBCOEFFS.
